...
def get_server(zeno: ZenoBackend):
    ...
    @api_app.post("/add-task", response_model=str, tags=["zeno"])
    def add_task(task:Task):
        task = zeno.add_task(task)
        return task
    ...
    def suggest_requirements(cur_info: SuggestNewReqRequest):
        requirements = zeno.suggest_requirements(cur_info)
        return requirements 
    ...
    @api_app.post("/remove-example", response_model=Dict[str, Requirement], tags=["zeno"])
    ...
    def update_evaluator(eval_feedback: EvaluatorFeedback):
        requirements = zeno.update_evaluator(eval_feedback)
        return requirements 
